...
async def search(request):
    ...
    search_type = (request.query.get('type') or 'video').strip().lower()
    try:
        max_results = int(request.query.get('max', yt_search.DEFAULT_MAX_RESULTS))
    except (TypeError, ValueError):
        raise web.HTTPBadRequest(reason="'max' must be an integer")
    if not query:
        raise web.HTTPBadRequest(reason="missing 'q' (search query)")
    if len(query) > 200:
        raise web.HTTPBadRequest(reason="'q' must be at most 200 characters")
    if search_type not in yt_search.SEARCH_TYPES:
        raise web.HTTPBadRequest(reason=f"'type' must be one of {sorted(yt_search.SEARCH_TYPES)}")
    max_results = max(1, min(max_results, yt_search.MAX_RESULTS_LIMIT))
    try:
        results = await asyncio.get_running_loop().run_in_executor(
            None, lambda: yt_search.search_youtube(query, search_type, max_results))
    except yt_search.SearchError as e:
        log.warning("Search failed for %r: %s", query, e)
        return web.Response(text=serializer.encode({'status': 'error', 'msg': str(e)}))
    except Exception:
        log.exception("Search crashed for %r", query)
        return web.Response(text=serializer.encode({'status': 'error', 'msg': 'Search failed'}))
    log.info("Search %r (%s): %d results", query, search_type, len(results))
    return web.Response(text=serializer.encode({
        'status': 'ok', 'query': query, 'type': search_type, 'results': results}))
...
async def trending(request):
    ...
    try:
        results = await asyncio.get_running_loop().run_in_executor(
            None, lambda: yt_search.get_trending(max_results))
    except yt_search.SearchError as e:
        log.warning("Trending failed: %s", e)
        return web.Response(text=serializer.encode({'status': 'error', 'msg': str(e)}))
    except Exception:
        log.exception("Trending crashed")
...
# Lightweight legacy UI for old browsers (see app/static/lite/). Registered
# before the frontend catch-all static route below.
@routes.get(config.URL_PREFIX + 'lite/')
async def lite_index(request):
    return web.FileResponse(_lite_file('lite.html'))

@routes.get(config.URL_PREFIX + 'lite/{name}')
async def lite_asset(request):
    name = request.match_info.get('name', '')
    if name not in ('lite.css', 'lite.js'):
        raise web.HTTPNotFound()
    return web.FileResponse(_lite_file(name))
